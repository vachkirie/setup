...
#
#------------------------------------------------------------------------------
# Reorder commits by domain
#------------------------------------------------------------------------------
#
def ReorderGitCommits(repo, onto, start, end):
    nb_reorder = 0
    clist = []
    clist_merged = []
    clist_exclude = []
    ...
    LogConsoleAndFile('\n')
    LogConsoleAndFile('Reorder %d Commits from %s..%s' % (len(clist), Sid(start), Sid(end)))

    for c in clist:
        if IsMarkedRight('[GOOGLE',repo.commit(c).summary): #exclude only GOOGLE patches
            clist_exclude.append(c)

    # initialize source to baseline
    repo.git.checkout(onto)
    ...
    for commit in clist:
        ...
        if commit in clist_merged: # if commit already merged
            continue

        if cbase_initialized == True:
            if CherryPick(repo, commit) == False:
                LogConsoleAndFile('ERROR: Reorder failed to cherry-pick %s' % (commit))
                sys.exit(0)
                continue
            src_cobj = repo.head.commit
        else: # no yet need to cherry-pick
            src_cobj = repo.commit(commit)

        clist_merged.append(commit) # mark the commit as merged

        if IsMarkedRight('[GOOGLE',src_cobj.summary):
        ...
        if len(dlist) == 0: # no domain identified
            dlist.append(unknown_domain)
            LogCpGeneric('[-]', commit, src_cobj.hexsha, src_cobj.summary, filelist, dlist)
        elif len(dlist)==1 and dlist[0]['path'] == 'unknown': # no domain identified
            LogCpGeneric('[-]', commit, src_cobj.hexsha, src_cobj.summary, filelist, dlist)
        elif len(dlist) > 1 : # several domain identified. Skip reordering. Requires split
            LogCpGeneric('[U]', commit, src_cobj.hexsha, src_cobj.summary, filelist, dlist)
        else:
            # if no-reoder option
            no_reorder = getDomainOption(dlist[0], "no-reorder")
            if no_reorder == True : # not ot be reordered
                LogCpGeneric('[ ]', commit, src_cobj.hexsha, src_cobj.summary, filelist, dlist)
                continue

            if interactive == True:
                 if AskIfReorder(dlist[0]['path']) == False:
                    # Then do not reorder domain
                    LogCpGeneric('[ ]', commit, src_cobj.hexsha, src_cobj.summary, filelist, dlist)
                    continue
            ...
            # if not yet existing, list commits from A to B
            #if clist_domain[0] != dlist[0]['path']:
            clist_domain[0] = dlist[0]['path']
            clist_domain[1] = gitGetCommitList(repo, start, end, dlist[0]['path'])

            logging.debug('candidates %s..%s %s' % (start, end,dlist[0]['path']))
            for f in clist_domain[1]:
                logging.debug('    %s - %s' % (Sid(f), repo.commit(f).summary))

            # has to be performed everytime as clist_merged can change
            ListStripList(clist_domain[1], clist_merged)
            ListStripList(clist_domain[1], clist_exclude)

            if len(clist_domain[1]) == 0: # if 0 commits to reorder
                logging.info('    empty list of commits to rorder')
                continue

            if clist_domain[1][0] == NextItem(clist, commit):
                logging.info('is next %s' % commit)
                #if next domain commit is also next commit in the global list
                continue

            clist_remove=[]
            for c in clist_domain[1]:
                dlist_temp = GetDomainsFromCommit(repo, c)

                if len(dlist_temp) > 1 :
                    clist_remove.append(c)
                    logging.debug('     unsplit %s - %s' % (Sid(c), repo.commit(c).summary))

                if dlist_temp[0]['path'] != dlist[0]['path']:
                    clist_remove.append(c)
                    logging.debug('%s wrong domain %s vs %s ' % (Sid(c), dlist_temp[0]['path'], dlist[0]['path']))

                no_reorder = getDomainOption(dlist_temp[0], "no-reorder")
                if no_reorder == True : # theoritacally could not happen
                    clist_remove.append(c)
                    logging.debug('no-reorder')

            ListStripList(clist_domain[1], clist_remove)

            logging.info('      choosen %d commits for %s' % (len(clist_domain[1]),dlist[0]['path']))
            for f in clist_domain[1]:
                logging.info('          %s - %s' % (Sid(f), repo.commit(f).summary))

            LogReorderPrepare(dlist[0], clist_domain[1], clist_merged)
            ...
            for c in clist_domain[1]:
                ...
                if len(dlist_temp) > 1 :
                    LogCpGeneric('[U]', c, src_cobj.hexsha, src_cobj.summary, [], dlist_temp)
                    continue

                if dlist_temp[0]['path'] != dlist[0]['path']:
                    logging.info('wrong domain %s vs %s ' % (dlist_temp[0]['path'], dlist[0]['path']))
                    #happens with subdomains
                    continue

                no_reorder = getDomainOption(dlist_temp[0], "no-reorder")
                if no_reorder == True : # theoritacally could not happen
                    LogCpGeneric('[x] skip no-reorder', commit, src_cobj.hexsha, src_cobj.summary, dlist_temp)
                    continue

                if CherryPick(repo, c) == False:
                    # cherry-pick failed likely due to a conflict
                    LogCpGeneric('[C] SKIPPED --- ', c, 'xxxx', repo.commit(c).summary, GetFlistFromCommit(repo, c), dlist_temp)
                    print ShellCmd('git status')
                    os.system('git reset -q HEAD')
                    os.system('git checkout -- .')
                    os.system('git clean -fd')
                ...
                src_cobj = repo.head.commit
                filelist = GetFlistFromCommit(repo, 'HEAD') # list files modified
                clist_merged.append(c) # mark the commit as merged
                LogCpGeneric('[R]', c, src_cobj.hexsha, src_cobj.summary, filelist)
                nb_reorder +=1
...
def LogHelp():
    ...
    print ''
    print 'Example:'
    print '     $ GitReorder.py --range HEAD~15..HEAD'
    print ''
...
# Get options
try:
   opts, args = getopt.getopt(sys.argv[1:],"hivj:r",["interactive","verbose","json=","range="])
...
nb_reorder = ReorderGitCommits(repo, rev_a, rev_a, head_split)
head_reorder=repo.head.commit.hexsha
...
LogConsoleAndFile('')
LogCommitRange('Reorder range:', rev_a, Sid(head_reorder))
